...
1. `context/project-overview.md` — what the product is, pages, flows, scope
2. `context/architecture.md` — stack, folder structure, data flow, deployment, invariants
3. `context/ui-tokens.md` — design tokens (`@theme` in globals.css)
4. `context/ui-rules.md` — layout, typography, component patterns
5. `context/ui-registry.md` — existing component classes (match before inventing)
6. `context/code-standards.md` — TypeScript, API routes, engine, PostHog events
7. `context/library-docs.md` — InsForge, DeepSeek, pdf-parse, Handlebars, Stripe, PostHog
8. `context/TEMPLATES.md` — LaTeX template storage, registry, adding templates
9. `context/build-plan.md` — phased features (build UI with mock data first)
10. `context/progress-tracker.md` — what is done, in progress, next

**Do not read or reference `context/aws-steps.md`** — personal AWS setup guide for the human operator only.
